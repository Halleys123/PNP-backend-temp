...
const changeForgotPassword = (0, utils_1.async_error_handler)((req, res, next) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    const foundUser = yield adminForgotPasswordTemp_1.AdminForgotPasswordTempModel.findById(tokenInfo._id);
    if (!foundUser)
        throw new utils_1.Custom_error({
    ...
    const admin = yield adminPerma_1.AdminModelPerma.findByIdAndUpdate(foundUser.adminId, {
        $set: { password: yield (0, utils_1.hashPassword)(newPassword) },
    });
    yield adminForgotPasswordTemp_1.AdminForgotPasswordTempModel.findByIdAndDelete(foundUser._id);
    req.admin = admin;
    next();
}));
